  ...
  constructor(props) {
    ...
    setupSocket() {
        ...
        websocket.on('reconnect', () => {
            ...
            notification.error({
        message: 'Version of client is not equal to server, please refresh.',
      })
        })
        ...
        websocket.on('open_event', evt => {
          ...
          localStorage.getItem(JSON.stringify(this.props.wsParam)),
        )
    ...
    leaveListener(e) {
        e.preventDefault()
        e.returnValue = '内容将被存储到缓存，下次打开相同场景优先从缓存获取！'
